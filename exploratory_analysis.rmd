...
test_map <- function(df, zoom) {
  ...
  df %>%
    ...
    addProviderTiles(providers$OpenStreetMap.DE) %>%
    setView(center_lng, center_lat, zoom) %>%
    addMarkers(lng = ~longitude, lat = ~latitude, icon = ~room_type_icons[room_type])
}


test_map(manhattan, 11)
