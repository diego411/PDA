...
sorted_earnings <- earnings_df[order(earnings_df$lowest_monthly_earnings), ]

# Calculate cumulative earnings
sorted_earnings$cumulative_earnings <- cumsum(sorted_earnings$lowest_monthly_earnings)

# view NYC's monthly earnings
cumulated_earnings <- max(sorted_earnings$cumulative_earnings)
cumulated_earnings
...
q_earnings_halfcut <- ecdf(sorted_earnings$cumulative_earnings)(cumulated_earnings_0.5)

# create vector indicating position in earnings distribution
q_value_halfcut <- quantile(sorted_earnings$lowest_monthly_earnings, probs = q_earnings_halfcut)
quantile_indicator <- ifelse(sorted_earnings$lowest_monthly_earnings > q_value_halfcut , "upper 4%", "lower 96%")
sorted_earnings <- cbind(sorted_earnings, quantile_indicator)

# prepare data for plotting
earningsplot_df <- sorted_earnings %>%
  group_by(quantile_indicator) %>%
  summarise(tot_earnings_mio = (sum(lowest_monthly_earnings)/1000000))
...
earningsplot_df %>%
  ...
  labs(
    title = "Cumulated Earnings of upper 4% and lower 96% of Monthly Earnings Distribution",
    y = "Cumulated Lowest Monthley Earnings in Mio",
    x = NULL
  )
...
The highest 4% in the earnings distribution earn as much as the 96% below them.
